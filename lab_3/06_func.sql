CREATE OR REPLACE FUNCTION lab_3.add_rating(
    p_user_id BIGINT,
    p_item_id BIGINT,
    p_rating SMALLINT,
...
RETURNS VOID AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM lab_3.users WHERE user_id = p_user_id) THEN
        RAISE EXCEPTION 'User not found';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM lab_3.content_items WHERE item_id = p_item_id) THEN
...
CREATE OR REPLACE FUNCTION lab_3.add_to_collection(
    p_user_id BIGINT,
    p_item_id BIGINT
)
RETURNS VOID AS $$
...
CREATE OR REPLACE FUNCTION lab_3.get_avg_rating(p_item_id BIGINT)
RETURNS NUMERIC AS $$
DECLARE
    result NUMERIC;
BEGIN
